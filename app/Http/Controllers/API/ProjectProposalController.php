...
class ProjectProposalController extends Controller
{
    ...
    public function options(Request $request): JsonResponse
    {
        ...
        return response()->json(['message' => 'Project proposal options retrieved successfully.', 'data' => $this->service->options($request->user())]);
    }

    public function store(StoreProjectProposalRequest $request): JsonResponse
    {
        return response()->json(['message' => 'Project proposal created successfully.', 'data' => $this->service->store($request->user(), $request->validated())], 201);
    }

    public function show(ProjectProposal $projectProposal): JsonResponse
    {
        $this->authorize('view', $projectProposal);

        return response()->json(['message' => 'Project proposal retrieved successfully.', 'data' => $this->withPermissions($projectProposal, request())]);
    }

    public function update(UpdateProjectProposalRequest $request, ProjectProposal $projectProposal): JsonResponse
    {
        return response()->json(['message' => 'Project proposal updated successfully.', 'data' => $this->service->update($projectProposal, $request->validated())]);
    }

    public function destroy(ProjectProposal $projectProposal): JsonResponse
    {
        $this->authorize('delete', $projectProposal);
        $this->service->delete($projectProposal);

        return response()->json(['message' => 'Project proposal deleted successfully.']);
    }

    public function submit(Request $request, ProjectProposal $projectProposal): JsonResponse
    {
        $this->authorize('submit', $projectProposal);

        return response()->json(['message' => 'Project proposal submitted for immediate head notation.', 'data' => $this->service->submit($projectProposal)]);
    }
    ...
    public function issueNtp(Request $request, ProjectProposal $projectProposal): JsonResponse
    {
        $this->authorize('issueNtp', $projectProposal);

        return response()->json(['message' => 'Notice to Proceed issued successfully.', 'data' => $this->service->issueNtp($projectProposal, $request->user())]);
    }

    public function downloadNtp(ProjectProposal $projectProposal)
    {
        $this->authorize('view', $projectProposal);
        abort_unless($projectProposal->noticeToProceed, 404, 'Notice to Proceed has not been issued.');
        $proposal = $this->service->find($projectProposal);

        return Pdf::loadView('pdf.notice-to-proceed', compact('proposal'))->download("{$proposal->noticeToProceed->ntp_number}.pdf");
    }
    ...
    public function uploadDocument(Request $request, ProjectProposal $projectProposal): JsonResponse
    {
        ...
        $path = $file->store("project-proposals/{$projectProposal->id}", $disk);
    ...
    {
        $this->authorize('view', $projectProposal);
        abort_unless($document->project_proposal_id === $projectProposal->id, 404);
        abort_unless(Storage::disk($document->disk)->exists($document->storage_path), 404, 'File not found.');
    ...
    public function deleteDocument(ProjectProposal $projectProposal, ProjectProposalDocument $document): JsonResponse
    {
        ...
        return response()->json(['message' => 'Supporting document deleted successfully.']);
    }

    private function withPermissions(ProjectProposal $proposal, Request $request): ProjectProposal
    {
        $proposal = $this->service->find($proposal);
        $user = $request->user();
