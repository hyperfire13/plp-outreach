<template>
    <MainLayout>
        ...
        <CrudModal
            ref="modalRef"
            :title="
                isEdit ? 'Edit Outreach Program' : 'Create Outreach Program'
            "
            :fields="fields"
            :form="form"
            :errors="errors"
...
const submit = async () => {
    ...
    try {
        const payload = {
            ...
            typical_budget:
                form.typical_budget === "" ? null : form.typical_budget,
            typical_duration_days:
...
const deleteProgram = async (program) => {
    const confirmed = confirm(`Delete ${program.name}?`);
    ...
    try {
        await outreachProgramService.remove(program.id);

        const currentPage = programs.value.current_page || 1;

        const remainingRows = (programs.value.data || []).length - 1;
